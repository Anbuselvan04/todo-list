        ...
        {props.list.map((ele) => {
          return <div className="list-element">
            <div className='upper-half'>
              {ele.edit ? <input type='text' className='search-bar' onKeyDown={(event)=>props.enter(event,ele)} onChange={(event) => props.change(event.target.value)}/> : <p className='task' style={{color : ele.isCompleted? "green" : "red"}}>{ele.value}</p>}
...
function App() {
  ...
  const enterHandler = (event, elem) =>{
    if(event.key === 'Enter'){
      setList(list.map((ele) => ele.id === elem.id ? {...ele, edit: false, value: task} : ele))
    }
  }
  const addTask = (task) =>{
      setList([...list, {
    ...
    <React.Fragment>
      <div className='main-container'>
        ...
        <div className='search'>
            <input type="text" placeholder="Add task" value={task} onChange={(events)=>setTask(events.target.value)} className='search-bar' />
